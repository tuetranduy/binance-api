...
class FutureController(BaseController):
    ...
    def place_future_order_hedge_mode(self, request):
        ...
        try:
            if order_type == 'LIMIT':
                ...
                result = self.client.futures_create_order(symbol=symbol,
                                                          side=side,
    ...
    def get_position_by_symbol(self, symbol):
        try:
            result = self.client.futures_position_information(symbol=symbol)
    ...
    def cancel_orders_by_symbol(self, request):
    ...
    def cancel_multiple_orders(self, request):
        app.logger.debug('cancel_multiple_orders_Requested payload: %s', request.form)

        self.keys.api_key = "" if self.keys.api_key is None else self.keys.api_key
        self.keys.secret_key = "" if self.keys.secret_key is None else self.keys.secret_key
        ...
        headers = {
            ...
            'X-MBX-APIKEY': self.keys.api_key if constants.TEST_MODE is False else constants.API_KEY
        }
        query = {'symbol': symbol, 'orderIdList': order_id_list, 'timeInForce': 'GTC'}

        payload = param_builder.create_params_with_signature(query, self.keys.secret_key)
    ...
    def close_position_for_normal_order(self, request):
        app.logger.debug('cancel_multiple_orders_Requested payload: %s', request.form)
        ...
        try:
            result = self.client.futures_create_order(symbol=symbol, side=side, type=order_type,
                                                      quantity=quantity)
    ...
    def close_position_for_hedge_order(self, request):
    ...
    def batch_close_position_for_normal_order(self, request):
    ...
    def set_tp_and_sl(self, request):
        symbol = request.form['symbol']
        side = request.form['side']
        position_side = request.form['positionSide']
        quantity = request.form['quantity']
        ...
        try:
            if order_type == "TAKE_PROFIT_MARKET" or order_type == "STOP_MARKET":
                ...
                response = self.client.futures_create_order(symbol=symbol, side=side,
                                                            positionSide=position_side,
                                                            quantity=quantity,
